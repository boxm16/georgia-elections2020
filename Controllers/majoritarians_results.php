...
//header('Location:errorPage.php');
    ...
    while ($row = $result->fetch_object()) {
        ...
        if ($districts_results[$districtId] == null) {
            $district = new District();
            $candidates = array();
            array_push($candidates, $candidate);
            $district->setDistrictCandidates($candidates);
            $districts_results[$districtId] = $district;
        } else {
...
$districts_results[31] = $district;

$district = new District();
$district->setMessage("ოკუპაციის გამო ამ ტერიტორიაზე არჩევნები არ ტარდება");
$districts_results[32] = $district;

$district = new District();
$district->setMessage("ოკუპაციის გამო ამ ტერიტორიაზე არჩევნები არ ტარდება");
$districts_results[33] = $district;

for ($x=1;$x<31;$x++) {
    $district=$districts_results[$x];
